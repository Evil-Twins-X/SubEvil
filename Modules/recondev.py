...
def recondev(Domains,useragent=useragent()):
    subdomains = []
    response = requests.get(f"https://recon.dev/api/search?key={recondev_api_key}&domain={Domains}", stream=True,verify=True,headers={'User-Agent':useragent},timeout=15)
